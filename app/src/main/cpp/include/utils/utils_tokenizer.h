...
private:
    ...
    static std::string whitespace_clean_and_low_case(std::string text) {
        text = boost::regex_replace(text, boost::regex(R"(\s+)"), " ");
        text = strip(text);
    ...
    std::set<std::pair<std::string, std::string>> getPairs(const std::vector<std::string> &word) {
    ...
    std::map<std::pair<std::string, std::string>, int> loadBpeRanks(const std::string &merge_path) {
    ...
    std::vector<std::string> str_bpe_ch(const std::string &token) {
        ...
        for (size_t i = 0; i < token.length();) {
            int cplen = 1;
            if ((token[i] & 0xf8) == 0xf0) cplen = 4;     // 占用4个字节，前5位为11110
            else if ((token[i] & 0xf0) == 0xe0) cplen = 3; // 占用3个字节，前4位为1110
            else if ((token[i] & 0xe0) == 0xc0) cplen = 2; // 占用2个字节，前3位为110
            // 个人感觉这行代码好像没什么用，如果三种情况都不符合，那么cplen就为初始化的0，是符合utf-8编码定义的
            if ((i + cplen) > token.length()) cplen = 1;
    ...
    std::vector<std::string> str_bpe_en(const std::string &token) {
        ...
        while (true) {
            ...
            for (size_t i = 0; i < word.size();) {
                if (i < word.size() - 1 && word[i] == min.first && word[i + 1] == min.second) {
    ...
    std::vector<std::string> word_piece(const std::string &token) {
        ...
        while (start < token.length()) {
            ...
            while (start < end) {
                ...
                if (tokenizer_token2idx.find(sub_str) != tokenizer_token2idx.end()) {
                    cur_substr = sub_str;
                    break;
                }
                end--;
...
public:
    ...
    int load(const std::string &path) {
        std::string vocab = path + "/stable_diffusion/utils/vocab.txt";
        if (is_ch_) {
            vocab = path + "/stable_diffusion/taiyi_onnx/vocab.txt";
        }
        if (!file_exists(vocab)) {
            LOGE("vocab %s not exist!", vocab.c_str());
            return -1;
        }
        ...
        std::string merge = path + "/stable_diffusion/utils/merges.txt";
        if (!file_exists(merge)) {
            LOGE("vocab %s not exist!", merge.c_str());
            return -1;
        }
    ...
    std::vector<int>
    tokenize_discard(std::string text, size_t max_length = 0, bool padding = false) {
    ...
    std::vector<int> encode(std::string text) {
        ...
        while (boost::regex_search(start, end, match_results, pat)) {
...
////            中文+英文
            ...
            for (const auto &bpe_token: tokens) {
                if (tokenizer_token2idx.find(bpe_token) != tokenizer_token2idx.end()) {
                    bpe_tokens.push_back(tokenizer_token2idx[bpe_token]);
                    token_strs.push_back(bpe_token);
                } else {
                    LOGD("unknown token : %s", bpe_token.c_str());
                }
            }
        ...
        ss << "]";
        LOGD("split prompt \"%s\" to tokens %s", original_text.c_str(), ss.str().c_str());
        return bpe_tokens;
    }
    ...
    //  ['.', 1.1]]
    std::vector<std::pair<std::string, float>> parse_prompt_attention(const std::string &prompt) {
        ...
        boost::regex re_attention(
                R"(\\\(|\\\)|\\\[|\\]|\\\\|\\|\(|\[|:([+-]?[.\d]+)\)|\)|]|[^\\()\[\]:]+|:)");
        boost::regex re_break(R"(\s*\bBREAK\b\s*)");
        ...
        boost::smatch m;
        std::string remaining_text = prompt;
        std::string::const_iterator start = remaining_text.begin();
        std::string::const_iterator end = remaining_text.end();

        while (boost::regex_search(start, end, m, re_attention)) {
            std::string text = m[0];
    ...
    std::pair<std::vector<int>, std::vector<float>> tokenize(const string text,
                                                             size_t max_length = 0,
                                                             bool padding = false) {
...

        {
            ...
            ss << "]";
            LOGD("parse '%s' to %s", text.c_str(), ss.str().c_str());
        }
